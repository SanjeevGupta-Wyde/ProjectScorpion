...
if ($vmostype -eq "windows"){
    Invoke-AzVMRunCommand -ResourceGroupName $envconfig.group.name -VMName $vmname -CommandId 'RunPowerShellScript' -ScriptPath "$VMOperationsPath\GetInstanceMatadataForWindowsVM.ps1" -Parameter @{param1 = "metadatavm"}
}
if ($vmostype -eq "linux"){
    Invoke-AzVMRunCommand -ResourceGroupName $envconfig.group.name -VMName $vmname -CommandId 'RunPowerShellScript' -ScriptPath "$VMOperationsPath\GetInstanceMatadataForLinuxVM.ps1" -Parameter @{param1 = "metadatavm"}
}
